(function(app){
    ...
    function Player() {
        ...
        if(localStorage.getItem('vkSessionAccessToken')) {
            if(localStorage.getItem('vkSessionExpires') == 0 || localStorage.getItem('vkSessionExpires') > Date.now()) {
...
//                    me.refreshLovedTracks();
    ...
    Player.prototype.vkAuth = function(win, callback){
        var me = this;
        ...
        VKApi.getSession(win, function(err, sess){
            if(!err) {
                app.analytics.vkAuthComplete();
                localStorage.setItem('vkSessionAccessToken', sess.accessToken);
                localStorage.setItem('vkSessionExpires', sess.expiresIn != 0 ? Date.now() + sess.expiresIn : 0 );
            ...
            if(app.currentAuthTab) {
                chrome.tabs.update(app.currentAuthTab.id, {
                    url:oldHref
                });
            }
        });
    ...
    Player.prototype.lastFmAuth = function(win, callback){
        var me = this;
...
//		LastFmApi.getToken(app.LASTFM_API_KEY, app.LASTFM_API_SECRET, function(err,token){
        LastFmApi.authToken(win, app.LASTFM_API_KEY, function(err, token) {
